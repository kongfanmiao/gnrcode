...
@timer
def band_structure(H, Erange=[-5, 5], index=False, figsize=(6, 4), **kwargs):
...
@timer
def dos(H, Erange=[-5, 5], figsize=(4, 6), **kwargs):
    ...
    plt.figure(figsize=figsize)
    plt.plot(bsav.DOS(E), E, **kwargs)
    plt.ylim(Erange[0], Erange[-1])
    plt.xlim(0, np.amax(bsav.DOS(E)) + 1)
    plt.ylabel('$E-E_F$ (eV)')
    plt.xlabel('DOS')
...
@timer
def ldos(H, location, Erange=[-5, 5], figsize=(1, 5),
         rescale=[0, 1], ret=False, **kwargs):
    """
    ...
    for i in range(len(eig_sub)):
        plt.hlines(eig_sub[i], 0, 1, alpha=ldos[i], **kwargs)
    plt.xticks([])
    plt.ylim(Emin, Emax)
    plt.ylabel('$E-E_F$ (eV)')
