...
@timer
def interpolated_bs(name, path, Hint=None, Hpr=None, Erange=[-5, 0],
                    figsize=(6, 4), tick_labels='XGX', overlap=False, npts=30,
                    marker_size=30, marker_color='g', marker='o', facecolors='none',
                    **kwargs):
    """
    ...
    if overlap:
        ...
        for j, ek_int in enumerate(eigh_int.T):
            plt.scatter(lk_int, ek_int, s=marker_size, marker=marker,
                        facecolors=facecolors, edgecolors=marker_color)
        plt.xlim(0, lk_pr[-1])
    else:
